...
class AdminLoginView(APIView):
    ...
    def post(self, request):
        ...
        if user.role not in ["admin", "superadmin"] and not (user.is_superuser or user.is_staff):
            return Response(
                {"message": "You are not authorized to access the admin panel. Only Admin accounts can login here."},
                status=status.HTTP_403_FORBIDDEN,
            )

        if (user.is_superuser or user.is_staff) and user.role not in ["admin", "superadmin"]:
            user.role = "admin"
